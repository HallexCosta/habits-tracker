...
export function HabitDay({ completed = 0, amount  = 0, date }: Props) {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger 
        className={clsx('w-10 h-10 border-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-violet-700 focus:ring-offset-2 focus:ring-offset-[#09090A]', {
          'bg-zinc-900  border-zinc-800': upProgress === 0,
          'bg-violet-500/80 border-violet-500/25': upProgress > 0 && upProgress < 20,
          'bg-violet-600/80 border-violet-600/25': upProgress >= 20 && upProgress < 40,
          'bg-violet-700/80 border-violet-700/25': upProgress >= 40 && upProgress < 60,
          'bg-violet-800/80 border-violet-800/25': upProgress >= 60 && upProgress < 80,
